...
newBeacons: set[Beacon] = set()

# We assume that scanner 0 is in (0, 0, 0).
# For each other scanner, we check the scanner overlapping with it,
# and move all the first scanner's beacons to the second scanner's coordinate system.
...
for conn in connections:
    tmpBeacons = set(scanners[conn])
    while conn != 0:
        (dest, rot, offset) = connections[conn]
        tmpBeacons = {add(reorient(p, rot), offset) for p in tmpBeacons}
        conn = dest
    newBeacons |= tmpBeacons

result = len(newBeacons)
